...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createDrinkProductCard(product) {
        ...
        const valueSpan = quantitySelector.querySelector('.quantity-value');
        let quantity = 1;

        // Helper to get supplement total
        function getSupplementsTotal() {
            let total = 0;
            // Checkboxes
            const qCheckboxes = supplementsSection.querySelectorAll('.checkbox-position');
            qCheckboxes.forEach(checkbox => {
                if (checkbox.checked) {
                    const supplementPrice = parseFloat(checkbox.dataset.price);
                    if (!isNaN(supplementPrice)) total += supplementPrice;
                }
            });
            // Whipped cream
            const qWhippedCreamItems = supplementsSection.querySelectorAll('.whipped-cream-item.selected');
            qWhippedCreamItems.forEach(item => {
                let priceText = item.querySelector('.pricesup')?.textContent || '';
                const price = parseFloat(priceText.replace(/[^\d.]/g, ''));
                if (!isNaN(price)) total += price;
            });
            // Syrups
            const qSyrupItems = supplementsSection.querySelectorAll('.syrup-item.selected');
            qSyrupItems.forEach(item => {
                let priceText = item.querySelector('.pricesup')?.textContent || '';
                const price = parseFloat(priceText.replace(/[^\d.]/g, ''));
                if (!isNaN(price)) total += price;
            });
        ...
        function updateAllPrices() {
            const supplementTotal = getSupplementsTotal();
            const total = (basePrice + supplementTotal) * quantity;
            priceElement.textContent = formatPrice(total) + ' dt';
            const clonedPrice = supplementsSection.querySelector('.cloned-price');
            if (clonedPrice) clonedPrice.textContent = formatPrice(total) + ' dt';
        }
        ...
        // Update price when supplements change
        const qCheckboxes = supplementsSection.querySelectorAll('.checkbox-position');
        qCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', updateAllPrices);
        });
        const qWhippedCreamItems = supplementsSection.querySelectorAll('.whipped-cream-item');
        qWhippedCreamItems.forEach(item => {
        ...
        const qSyrupItems = supplementsSection.querySelectorAll('.syrup-item');
        qSyrupItems.forEach(item => {
            item.addEventListener('click', () => {
                item.classList.toggle('selected');
                updateAllPrices();
        ...
        // Reset quantity and price on mouse leave
        productCard.addEventListener('mouseleave', () => {
            quantity = 1;
            valueSpan.textContent = quantity;
            updateAllPrices();
    ...
    function formatPrice(price) {
        // If integer, show as is. If decimal, show up to 1 decimal, remove trailing zero.
